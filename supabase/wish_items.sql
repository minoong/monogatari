...
create policy "wish items are insertable by anonymous users"
  on public.wish_items for insert to anon with check (true);

insert into storage.buckets (id, name, public)
values ('wish-images', 'wish-images', true)
on conflict (id) do update set public = true;

drop policy if exists "wish images are readable by anonymous users" on storage.objects;
create policy "wish images are readable by anonymous users"
  on storage.objects for select to anon using (bucket_id = 'wish-images');
...
create policy "wish images are insertable by anonymous users"
  on storage.objects for insert to anon with check (bucket_id = 'wish-images');
