...
export const ExpenseActivity: React.FC = () => {
  ...
  const filtered = useMemo(() => expenses.filter((expense) => {
    ...
    const matchesSearch = !deferredSearch || [expense.item_name, expense.merchant, expense.memo].some((value) => value?.toLocaleLowerCase("ko-KR").includes(deferredSearch));
  ...
  return <AppScreen appBar={{ title: "여행 가계부" }}>
    <main className="min-h-full bg-slate-50 pb-[calc(6rem+max(env(safe-area-inset-bottom,0px),12px))] dark:bg-slate-950">
      <Tabs aria-label="가계부 보기" defaultSelectedKey="list" className="w-full">
        ...
        <Tabs.Panel className="!p-0" id="list">
          {query.isLoading ? <ExpenseSkeleton /> : initialError ? <LoadError onRetry={() => query.refetch()} /> : <section className="mx-auto max-w-lg px-4 py-4">
        ...
        <Tabs.Panel className="!p-0" id="stats">
          {query.isLoading ? <div className="mx-auto max-w-lg px-4 py-4"><ChartSkeleton /></div> : initialError ? <LoadError onRetry={() => query.refetch()} /> : <section className="mx-auto max-w-lg px-4 py-4">{expenses.length ? <ExpenseCharts expenses={filtered} /> : <EmptyState onCreate={openCreate} />}</section>}
        </Tabs.Panel>
      </Tabs>
    </main>
    ...
    <FilterDrawer category={category} from={from} open={filterOpen} person={person} to={to} onCategory={setCategory} onFrom={setFrom} onOpenChange={setFilterOpen} onPerson={setPerson} onReset={clearFilters} onTo={setTo} />
...
function FilterDrawer({ open, person, category, from, to, onOpenChange, onPerson, onCategory, onFrom, onTo, onReset }: { open: boolean; person: PersonFilter; category: CategoryFilter; from: string; to: string; onOpenChange: (open: boolean) => void; onPerson: (value: PersonFilter) => void; onCategory: (value: CategoryFilter) => void; onFrom: (value: string) => void; onTo: (value: string) => void; onReset: () => void }) { return <Drawer open={open} onOpenChange={onOpenChange}><DrawerPopup variant="inset" showBar><DrawerHeader className="pt-6"><DrawerTitle>지출 필터</DrawerTitle></DrawerHeader><DrawerPanel className="space-y-5"><FilterGroup label="사용자"><div className="grid grid-cols-3 gap-2">{(["all", ...EXPENSE_PEOPLE] as PersonFilter[]).map((value) => <FilterChoice key={value} selected={person === value} onClick={() => onPerson(value)}>{value === "all" ? "전체" : EXPENSE_PERSON_META[value].label}</FilterChoice>)}</div></FilterGroup><FilterGroup label="카테고리"><div className="grid grid-cols-3 gap-2">{(["all", ...EXPENSE_CATEGORIES] as CategoryFilter[]).map((value) => <FilterChoice key={value} selected={category === value} onClick={() => onCategory(value)}>{value === "all" ? "전체" : EXPENSE_CATEGORY_META[value].label}</FilterChoice>)}</div></FilterGroup><FilterGroup label="날짜 범위"><div className="grid grid-cols-2 gap-2"><input aria-label="시작 날짜" className="h-11 rounded-xl border px-3 text-sm dark:bg-slate-900" max={to || undefined} onChange={(event) => onFrom(event.target.value)} type="date" value={from} /><input aria-label="종료 날짜" className="h-11 rounded-xl border px-3 text-sm dark:bg-slate-900" min={from || undefined} onChange={(event) => onTo(event.target.value)} type="date" value={to} /></div></FilterGroup></DrawerPanel><DrawerFooter className="grid grid-cols-2 gap-2 pb-[calc(1rem+env(safe-area-inset-bottom))]"><button className="h-12 rounded-xl bg-slate-100 font-bold dark:bg-slate-800" onClick={onReset} type="button">초기화</button><button className="h-12 rounded-xl bg-slate-900 font-bold text-white dark:bg-white dark:text-slate-900" onClick={() => onOpenChange(false)} type="button">적용</button></DrawerFooter></DrawerPopup></Drawer>; }
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) { return <section><p className="mb-2 text-sm font-bold">{label}</p>{children}</section>; }
function FilterChoice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) { return <button className={cn("min-h-11 rounded-xl border px-2 text-xs font-bold", selected ? "border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-500/15" : "border-slate-200 dark:border-slate-700")} onClick={onClick} type="button">{children}</button>; }
