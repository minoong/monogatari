...
function FilterChoice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) { return <button className={cn("min-h-11 min-w-0 truncate rounded-xl border px-2 text-xs font-bold", selected ? "border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-500/15" : "border-slate-200 dark:border-slate-700")} onClick={onClick} title={typeof children === "string" ? children : undefined} type="button">{children}</button>; }
