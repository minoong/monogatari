  ...
  return <article data-passenger-ticket className="relative overflow-hidden rounded-[26px] border border-[#d7e5f2] bg-white shadow-[0_18px_38px_-30px_rgba(7,47,100,0.66)]">
    ...
    <div className="px-4 pb-5 pt-4 sm:px-5">
      ...
      <div className="relative mt-5 space-y-4 pl-12 md:hidden" aria-label={`${flight.duration} 비행`}>
        ...
        <div className="grid grid-cols-[1fr_auto] items-start gap-4"><div><div className="flex items-baseline gap-1.5"><p className="text-3xl font-black text-[#0b3478]"><FlightTime value={flight.arrival.time} /></p>{flight.arrival.nextDay ? <span className="text-xs font-bold text-[#3e9468]">+1일</span> : null}</div><p className="mt-1 text-[11px] font-semibold leading-4 text-slate-500">{flight.arrival.airport}{flight.arrival.terminal ? ` · ${flight.arrival.terminal}` : ""}</p></div><p className="pt-1 text-xl font-black text-[#0b3478]">{flight.arrival.code}</p></div>
      </div>
