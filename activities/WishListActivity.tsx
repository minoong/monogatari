...
export const WishListActivity: React.FC<WishListActivityProps> = ({ params }) => {
  ...
  return (
    <AppScreen appBar={{ title: meta.title }}>
      <main className="flex-1 h-full min-h-full bg-slate-50 pb-[calc(1.5rem+env(safe-area-inset-bottom,16px))] dark:bg-slate-950">
        ...
        <Button aria-label={`${meta.title} 등록`} className="fixed bottom-[calc(1.5rem+env(safe-area-inset-bottom,16px))] right-5 z-40 h-14 min-w-14 rounded-full px-5 shadow-xl" onPress={openCreateDrawer}><Plus className="size-5" /><span className="font-bold">등록</span></Button>
      </main>
