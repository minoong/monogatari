  ...
  return <div ref={cardRef} className="min-w-0">
    <MorphingDialog transition={{ type: "spring", bounce: 0.08, duration: 0.45 }}>
      <article className={cn("relative overflow-hidden rounded-2xl border bg-white shadow-sm transition dark:bg-slate-900", current ? "border-blue-400 ring-2 ring-blue-100 dark:border-blue-400 dark:ring-blue-500/20" : "border-slate-200 dark:border-slate-800")}>
        <MorphingDialogTrigger ariaLabel={`${item.title} 자세히 보기`} className="group block min-h-28 w-full rounded-2xl p-4 pb-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
        ...
        {item.google_maps_url && <div className="flex h-9 items-center justify-end px-4 pb-3"><a aria-label={`${item.title} Google Maps 열기`} href={item.google_maps_url} target="_blank" rel="noreferrer" className="inline-flex size-7 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:text-slate-500 dark:hover:bg-slate-800 dark:hover:text-slate-200"><MapPinIcon size={18} /></a></div>}
      </article>
