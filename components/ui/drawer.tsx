      ...
      <DrawerViewport position={position} variant={variant}>
        <DrawerPrimitive.Popup
          className={cn(
            ...
            position === "bottom" &&
              "data-nested-drawer-open:transform-[translateY(calc(var(--drawer-swipe-movement-y,0px)_-_var(--stack-peek-offset)_-_(var(--shrink)_*_var(--height))))_scale(var(--scale))] origin-[50%_calc(100%_-_var(--inset))]",
            position === "top" &&
              "data-nested-drawer-open:transform-[translateY(calc(var(--drawer-swipe-movement-y,0px)_+_var(--stack-peek-offset)_+_(var(--shrink)_*_var(--height))))_scale(var(--scale))] origin-[50%_var(--inset)]",
            position === "left" &&
              "data-nested-drawer-open:transform-[translateX(calc(var(--drawer-swipe-movement-x,0px)_+_var(--stack-peek-offset)))_scale(var(--scale))] origin-right",
            position === "right" &&
              "data-nested-drawer-open:transform-[translateX(calc(var(--drawer-swipe-movement-x,0px)_-_var(--stack-peek-offset)))_scale(var(--scale))] origin-left",
            className,
          )}
...
export function DrawerFooter({
  ...
  const defaultProps = {
    className: cn(
      "flex flex-col-reverse gap-2 px-6 pb-[var(--sa-bottom)] sm:flex-row sm:justify-end",
      !allowSelection && "cursor-default",
      {
        bare: "in-[[data-slot=drawer-popup]:has([data-slot=drawer-panel])]:pt-3 pt-4 pb-[calc(var(--sa-bottom)_+_--spacing(6))]",
        default: "border-t bg-muted/72 pt-4 pb-[calc(var(--sa-bottom)_+_--spacing(4))]",
      }[variant || "default"],
      className,
    ),
