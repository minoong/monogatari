...
export function WishImagePicker({ images, onChange }: WishImagePickerProps) {
  const sensors = useSensors(
    useSensor(MouseSensor, { activationConstraint: { distance: 8 } }),
    useSensor(TouchSensor, { activationConstraint: { delay: 250, tolerance: 5 } }),
  );
  const inputId = "wish-images";
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div><p className="text-sm font-semibold text-slate-800 dark:text-slate-100">이미지</p><p className="mt-0.5 text-xs text-slate-400">첫 사진이 대표 이미지예요 · 드래그해서 순서를 바꿀 수 있어요.</p></div>
      ...
      <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
        <SortableContext items={images.map((image) => image.id)} strategy={rectSortingStrategy}>
          <div className="grid grid-cols-3 gap-2">
            {images.map((image, index) => <SortableImage key={image.id} image={image} index={index} onRemove={() => onChange(images.filter((item) => item.id !== image.id))} />)}
...
function SortableImage({ image, index, onRemove }: { image: WishImageDraft; index: number; onRemove: () => void }) {
  ...
  return <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={`relative aspect-square overflow-hidden rounded-2xl border bg-slate-100 ${isDragging ? "z-10 scale-105 opacity-70 shadow-xl" : "border-slate-200 dark:border-slate-700"}`}>
    <Image alt={index === 0 ? "대표 이미지" : `위시 이미지 ${index + 1}`} className="object-cover" fill sizes="33vw" src={image.url} unoptimized />
    ...
    <button aria-label={`이미지 ${index + 1} 삭제`} className="absolute right-1.5 top-1.5 flex size-7 items-center justify-center rounded-full bg-black/65 text-white" onClick={onRemove} type="button"><X className="size-4" /></button>
    <button
      aria-label={`이미지 ${index + 1} 순서 변경`}
