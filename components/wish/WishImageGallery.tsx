...
export function WishImageGallery({ images, title, onImagePress }: { images: WishImage[]; title: string; onImagePress: (index: number) => void }) {
  ...
  return <Swiper modules={[Navigation, Pagination, A11y]} navigation pagination={{ clickable: true }} onSlideChange={() => undefined} className="wish-image-gallery h-56" a11y={{ prevSlideMessage: "이전 사진", nextSlideMessage: "다음 사진", paginationBulletMessage: "사진 {{index}}로 이동" }}>
