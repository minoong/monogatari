...
export interface DriftWallProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** 배경 장식용 — 상호작용·lazy 로딩·포커스 링 비활성화 */
  decorative?: boolean;
...
const DriftWall = ({
  ...
  const columnMeta = useMemo<ColumnMeta[]>(() => {
    ...
    const height = containerHeight > 0 ? containerHeight : 600;
    return columnItems.map(col => {
      const copyHeight = Math.max(unit, col.length * unit);
      const copies = Math.max(3, Math.ceil((height * 2) / copyHeight) + 2);
  ...
  useLayoutEffect(() => {
    ...
    const syncHeight = () => {
      ...
      if (next <= 0) return;
      setContainerHeight((prev) => (Math.abs(prev - next) > 12 ? next : prev || next));
    };
  ...
  useEffect(() => {
    offsetsRef.current = columnMeta.map((meta, c) => {
      const prev = offsetsRef.current[c];
      if (prev != null && Number.isFinite(prev)) {
        return ((prev % meta.copyHeight) + meta.copyHeight) % meta.copyHeight;
      }
      return meta.copyHeight * ((c * 0.37) % 1);
    });
    if (velocitiesRef.current.length !== columnItems.length) {
  ...
  const tileClass = cx(
    'group/tile relative block flex-none outline-none',
    !decorative && 'cursor-pointer',
  ...
  const imgClass = cx(
    'block h-full w-full select-none object-cover bg-[var(--dw-tile-bg)]',
    '[filter:grayscale(var(--dw-gray))_saturate(0.92)]',
    !decorative && 'transition-[filter] duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)]',
    !decorative &&
  ...
  const overlayClass = cx(
    ...
    !decorative && 'transition-opacity duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)]',
    !decorative && 'group-[.is-active]/tile:opacity-0 group-focus-visible/tile:opacity-0'
  );
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    ...
    return (
      <div key={id} aria-hidden={decorative ? true : undefined} {...commonProps}>
        {inner}
      </div>
    );
  };
  ...
  return (
    <div
      ref={containerRef}
      className={cx('relative h-full w-full overflow-hidden [transform:translateZ(0)]', className)}
      style={cssVars}
      ...
      onPointerLeave={decorative ? undefined : handlePointerLeaveWall}
    ...
    >
      <div
        ref={planeRef}
        className="absolute left-1/2 top-1/2 flex flex-row [transform-style:preserve-3d] [transform-origin:50%_50%] [backface-visibility:hidden] will-change-transform"
      >
        {columnItems.map((col, c) => {
          ...
          return (
            ...
            >
              <div
                className="flex flex-col [transform-style:preserve-3d] [backface-visibility:hidden] will-change-transform"
                ref={el => {
